...
function showMessage() {
	alert("죄송합니다. 현재 개인 구매 페이지는 통장 입금만 지원하고 있습니다.");
...
<br/>
<div class="container">
<c:set var="targetUrl"><c:url value="/accessory/newPtpOrderSubmitted.do" /></c:set>
<form:form modelAttribute="myOrder"  action="${targetUrl}" method="post">
	...
	<div id = "itemInfo">
		<table class="table table-striped">
    		...
    		<tbody>
    			<tr>
       				<td rowspan = "4" style="width : 30%;" > <img src="${pitem.item.image}" width="300" height="250"> </td>
      			</tr>
      			<tr>
				...
				<tr>
					...
					<td> ${pitem.item.price} + ${pitem.devP} = <fmt:formatNumber value="${myOrder.totalPrice}" pattern="###,###,###"/> 원 </td>
				</tr>
   			 </tbody>
  		</table>
	</div>
		...
		<table class="table table-striped">
	  		...
	  		<tbody>	
	  			<tr>
					...
					<td>
						<form:radiobutton path="payType" id="bank" value = "0" label="무통장 입금" checked="true" />
						<form:radiobutton path="payType" value = "1" label="카드 결제" onclick="showMessage()"/>
					</td>
				</tr>
				<tr>
					<td style="width : 32%;" ><label> 입금자명 </label> </td> 
					<td><input type="text" name="depositor" placeholder="ex : ${myOrder.ptpOrder.depositor}" /></td>
				</tr>
			</tbody>
		</table>
	</div>
